                        ...
                        for entry in endpoint_detail:
                            ...
                            if entry['device_id'] not in instance_list and \
                               server.status == 'ACTIVE':
                                ...
                                # check for vms in range and issue ping signal   
                                #for ip in ips:
                                    #if ip in ipset:
                                        #print("instance %s found"%(entry['ip_address']))
                                        #ip_net=ipaddress.ip_address(uentry['ip_address'])
                                        #output = subprocess.Popen(['ping', '-c','2',entry['ip_address']],stdout=subprocess.PIPE)
                                        #myout=output.communicate()[0]
                                        #print myout
