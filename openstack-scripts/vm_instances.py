...
keystone = ksclient.Client(auth_url="http://172.168.2.62:35357/v2.0",
                           username="admin",
                           password="admin",
                           tenant_name="admin")
...
glance=glance_client.Client(endpoint="http://172.168.2.62:9292", token=token)
 

#if not nova.keypairs.findall(name="admin"):
    #ith open(os.path.expanduser('~/.ssh/id_rsa.pub')) as fpubkey:
